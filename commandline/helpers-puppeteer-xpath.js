async function waitForElement(page, xpath, timeout = 5000) {
  console.log(`Waiting for xpath to appear: ${xpath}`);
...
async function waitForElementToDisappear(page, xpath, timeout = 5000) {
  console.log(`Waiting for xpath to disappear: ${xpath}`);
  const start = Date.now();
...
async function puppeteerWaitForXpath(page, xpath, timeout = 5000) {
  console.log(`Waiting for xpath to appear: ${xpath}`);
  const start = Date.now();
...
async function puppeteerWaitForAbsentXpath(page, xpath, timeout = 3000) {
  console.log(`Waiting for xpath to disappear: ${xpath}`);
  const start = Date.now();
